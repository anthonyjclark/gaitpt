...
class KinematicsNoiseTransform(object):
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        # Add noise to angles
        new_tensor[: self.num_gauss] += (
            torch.randn_like(tensor[: self.num_gauss]) * self.std + self.mean
        )

        # Randomly flip touch sensors (and don't do anything with the sinusoid)
        new_tensor[self.touch_index : -1] = torch.randint_like(
            tensor[self.touch_index : -1], 0, 2
        )
...
class KinematicsDataset(Dataset):
    def __init__(self, csv_path: Path, transform=None):
        ...
        time_step = 0.02
        df["Sine"] = np.sin(
            2 * np.pi * frequency * np.arange(0, len(df) * time_step, time_step)
        )
...
datasets = {
    f.stem.split("_")[0]: KinematicsDataset(f, transform=transform)
    for f in DATA_DIR.glob("*_kinematic.csv")
}
